...
export default function AgendaPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12">
      ...
      {/* ── Google Calendar Panel (Admin only) ── */}
      {isAdmin && gcalStatus !== null && (
        <div className={`rounded-2xl border px-5 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-all
          ${gcalStatus.connected
            ? "bg-emerald-500/5 border-emerald-500/30"
            : "bg-[#0A101D]/40 border-slate-800"
          }`}
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-white flex items-center gap-2">
                Google Calendar
                {gcalStatus.connected && (
                  <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 text-[10px] font-black border border-emerald-500/30">
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                {gcalStatus.connected
                  ...
                  : gcalStatus.configured
                    ...
                    : "Configura GOOGLE_CLIENT_ID y GOOGLE_CLIENT_SECRET en el .env del servidor para activar esta función."
                }
              </p>
            </div>
          </div>
          ...
          {gcalStatus.configured && (
            gcalStatus.connected ? (
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-300 text-xs font-bold hover:bg-rose-500/20 transition-all shrink-0 disabled:opacity-50"
              >
                <Unlink size={13} />
              ...
              <button
                ...
                disabled={gcalLoading}
                className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-[#20CDFE] to-[#1ED1B4] text-[#07060B] text-xs font-extrabold hover:opacity-90 shadow-md shadow-[#20CDFE]/20 transition-all shrink-0 disabled:opacity-50"
              >
                <Link2 size={13} />
                {gcalLoading ? "Redirigiendo..." : "Conectar Google Calendar"}
              </button>
            )
          )}
        </div>
